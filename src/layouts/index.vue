<template>
  <div class="shark-admin-wrapper" :class="classObj">
    <div
    ...
    >
      ...
      <div class="sk-main main-padding">
        <!-- <sk-ad /> -->
        <sk-app-main />
      </div>
    </div>
    ...
    >
      ...
      />
      <sk-side-bar />
      <div class="sk-main" :class="collapse ? 'is-collapse-main' : ''">
        <div :class="header === 'fixed' ? 'fixed-header' : ''">
          <sk-nav-bar />
          <sk-tabs-bar v-if="tabsBar === 'true' || tabsBar === true" />
        </div>
        <!-- <sk-ad /> -->
        <sk-app-main />
      </div>
  ...
  export default {
    ...
    mounted() {
      ...
      if (userAgent.includes('Juejin')) {
        this.$baseAlert(
          'shark-admin不支持在掘金内置浏览器演示，请手动复制以下地址到浏览器中查看http://mpfhrd48.sanxing.uz7.cn/shark-admin'
        )
      }
